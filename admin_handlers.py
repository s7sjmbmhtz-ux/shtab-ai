...
from admin_states import AdminBroadcastStates, AdminPackageStates, AdminUserStates
...
async def admin_stats(callback: CallbackQuery) -> None:
    async with db_manager.connection() as conn:
        ...
        ai_cost = float((await (await conn.execute(
...
async def admin_model_card(callback: CallbackQuery) -> None:
    ...
    if model.kind == GenerationKind.VIDEO:
        ...
        text += f"Базовая цена: <b>{int(db_settings.get('token_cost', model.token_cost))} 💎</b>"
    await callback.answer()
