...
def admin_main_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="🧾 Очередь", callback_data="admin:jobs"),
...
def model_card_keyboard(model_key: str, *, enabled: bool, kind: str) -> InlineKeyboardMarkup:
    ...
    price_row = [
        InlineKeyboardButton(text="₽ Себестоимость", callback_data=f"admin:model_cost:{model_key}"),
    ]
    if kind != GenerationKind.VIDEO.value:
        price_row.insert(
            0,
            InlineKeyboardButton(text="💎 Цена", callback_data=f"admin:model_price:{model_key}"),
        )
    return InlineKeyboardMarkup(inline_keyboard=[
        price_row,
        [InlineKeyboardButton(text="📈 Минимальная маржа", callback_data=f"admin:model_margin:{model_key}")],
...
def generation_jobs_keyboard(rows: list[dict]) -> InlineKeyboardMarkup:
    buttons: list[list[InlineKeyboardButton]] = []
    for row in rows:
        icon = "⏳" if row["status"] == "processing" else "📦" if row["status"] == "ready" else "✅"
        buttons.append([
            InlineKeyboardButton(
                text=f"{icon} #{row['id']} · {row['model_key']} · {row['status']}",
                callback_data=f"admin:job:{row['id']}",
            )
        ])
    buttons.append([InlineKeyboardButton(text="🔄 Обновить", callback_data="admin:jobs")])
    buttons.append([InlineKeyboardButton(text="⬅️ В админ-панель", callback_data="admin:main")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)


def generation_job_card_keyboard(job_id: int, status: str) -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    if status in {"processing", "ready"}:
        rows.append([
            InlineKeyboardButton(text="🔄 Проверить сейчас", callback_data=f"admin:job_retry:{job_id}")
        ])
        rows.append([
            InlineKeyboardButton(text="↩️ Вернуть списание", callback_data=f"admin:job_refund:{job_id}")
        ])
    rows.append([InlineKeyboardButton(text="⬅️ К очереди", callback_data="admin:jobs")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
