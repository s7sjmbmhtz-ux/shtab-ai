...
from access_middleware import BlockedUserMiddleware, RateLimitMiddleware
...
from services.fsm_storage import fsm_storage
...
from support_handlers import admin_router as support_admin_router, router as support_router
...
class Application:
    ...
    async def initialize(self) -> None:
        ...
        await fsm_storage.cleanup_stale()

        self.bot = Bot(
            token=settings.BOT_TOKEN,
            default=DefaultBotProperties(parse_mode=ParseMode.HTML),
        )
        public_media_service.set_payment_notifier(self._notify_payment)
        await public_media_service.start()
        await generation_job_service.start(self.bot)
        await operations_service.start(self.bot)
        self.dp = Dispatcher(
            storage=fsm_storage,
        ...
        self.dp.message.outer_middleware(RateLimitMiddleware())
        self.dp.callback_query.outer_middleware(RateLimitMiddleware())

        # Более узкие роутеры подключаются раньше общего меню.
        self.dp.include_router(support_admin_router)
        self.dp.include_router(admin_router)
        self.dp.include_router(token_admin_router)
        self.dp.include_router(payment_router)
        self.dp.include_router(history_router)
    ...
    async def _notify_payment(self, order) -> None:
        if self.bot:
            ...
            try:
                ...
                for order in credited:
                    with contextlib.suppress(Exception):
                        await self._notify_payment(order)
    ...
    async def shutdown(self, signame: str = "") -> None:
        ...
        try:
            await generation_job_service.stop()
        except Exception as exc:
            logger.error("Ошибка остановки восстановления генераций: %s", exc)

        try:
            await operations_service.stop()
        except Exception as exc:
            logger.error("Ошибка остановки мониторинга: %s", exc)
