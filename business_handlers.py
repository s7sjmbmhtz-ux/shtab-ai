...
from business_keyboards import (
    ...
    marketplace_platform, marketplace_product_type, marketplace_style, marketing_menu, sales_menu,
)
...
from marketplace_prompts import build_prompts
...
async def marketplace_product_name(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("Перечислите только реальные характеристики и преимущества товара.")
...
async def marketplace_generate(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    prompts=build_prompts(data["product_type"], data["product_name"], data["features"], data["style"], data["goal"], data["platform"], has_photo=input_image is not None)
    status=await callback.message.answer(f"⏳ Создаю {len(prompts)} изображение(я)…")
    model_by_type={"main":"marketplace-main","lifestyle":"marketplace-lifestyle","infographic":"marketplace-infographic","bundle":"marketplace-bundle-item"}
    try:
        for index,prompt in enumerate(prompts,1):
            charge,task=await generation_service.create_media_task(callback.from_user.id, model_by_type[data["product_type"]], prompt, input_image=input_image, overrides={"width":1024,"height":1280,"strength":0.42 if input_image else 0.72})
