...
MARKETPLACE_BRAND_CAPTION = "📦 Карточка создана в ШТАБ AI\\n👉 @ShtabProBot"
...
async def marketplace_generate(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        for index,prompt in enumerate(prompts,1):
            ...
            if not urls:
                logger.warning("Marketplace: пустой результат, повторная попытка %s", index)
                retry_overrides = dict(overrides)
                retry_overrides["seed"] = int(retry_overrides.get("seed", 0)) + 999
                _, task = await generation_service.create_media_task(
                    callback.from_user.id,
                    model_by_type[data["product_type"]],
                    prompt,
                    input_image=input_image,
        ...
        await status.edit_text("✅ Готово. Для комплекта использованы три разных промпта и разные параметры изменения исходного фото.")
    except InsufficientBalanceError as exc:
