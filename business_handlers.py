...
from services.genapi_client import GenAPIError, GenAPITaskFailedError
...
async def _marketplace_generate_locked(
    ...
    try:
        ...
        for index, prompt in enumerate(prompts, 1):
            ...
            delivered = await _generate_marketplace_image(
                callback,
                prompt=prompt,
                model_key=model_key,
        ...
        if pending_count:
            await status.edit_text(
                "⌛ Часть изображений ещё создаётся. Задания сохранены, "
                "результаты придут автоматически после готовности."
            )
        else:
            await status.edit_text(
    ...
    for attempt in range(2):
        ...
        try:
            ...
            job_id = await generation_job_service.register(
                ...
                prompt=prompt,
                generation_settings={
                    "business_mode": "marketplace",
                    "media_overrides": current_overrides,
                    "has_input_image": input_image is not None,
                },
            )
            urls = await generation_job_service.wait_for_result(job_id, task)
            await callback.message.answer_photo(urls[0], caption=caption)
            ...
            charge = None
            logger.warning(
                "Marketplace: ошибка результата, попытка %s/2: %s",
                attempt + 1,
                exc,
            )
            if attempt == 1:
            ...
            if charge is not None:
                await billing_service.refund(charge, "Ошибка карточки маркетплейса")
            raise
        finally:
