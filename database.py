...
class DatabaseConnectionManager:
    ...
    async def _create_tables(self) -> None:
        ...
        # Состояния многошаговых диалогов переживают перезапуск бота.
        await self._pool.execute("""
        ...
        """)
        await self._pool.execute(
            "CREATE INDEX IF NOT EXISTS idx_fsm_storage_updated "
        ...
        await self._pool.execute(
            ...
            "ON model_health_events(model_key, created_at)"
        )
        ...
        await self._ensure_column("model_prices", "disabled_reason", "TEXT")
        await self._ensure_column("model_prices", "auto_disabled_at", "DATETIME")
        await self._ensure_column("model_prices", "auto_disabled_until", "DATETIME")
    ...
    async def check_rate_limit(
        ...
        *,
        ...
        """Атомарно учитывает действие и возвращает допуск/секунды ожидания."""
        ...
        async with self.connection() as conn:
            ...
            try:
                ...
                if count >= max(1, int(limit)):
                    retry_after = max(
                        1,
                        window - (now - int(row["window_started_at"])),
                    )
                    await conn.commit()
    ...
    async def refund_generation(
        ...
        async with self.connection() as conn:
            ...
            try:
                ...
                await conn.execute(
                    """INSERT INTO funnel_events (user_id, event, context_json)
                       VALUES (?, 'generation_failed', ?)""",
                    (
                        user_id,
                        json.dumps(
                            {"model_key": model_key, "kind": generation_kind},
                            ensure_ascii=False,
                        ),
                    ),
                )
    ...
    async def complete_generation(
        ...
        *,
        ...
        async with self.connection() as conn:
            await conn.execute("BEGIN IMMEDIATE")
            row = await (await conn.execute(
                "SELECT user_id, model_key, kind FROM generation_economics WHERE id=?",
                (ledger_id,),
            )).fetchone()
            cursor = await conn.execute(
                """UPDATE generation_economics
            ...
            if cursor.rowcount == 1 and row:
                await conn.execute(
                    """INSERT INTO funnel_events (user_id, event, context_json)
                       VALUES (?, 'generation_completed', ?)""",
                    (
                        int(row["user_id"]),
                        json.dumps(
                            {"model_key": row["model_key"], "kind": row["kind"]},
                            ensure_ascii=False,
                        ),
                    ),
                )
        ...
        async with self.connection() as conn:
            ...
            await conn.execute(
                """INSERT INTO generation_history
                   (user_id, economic_id, model_key, kind, prompt,
                    settings_json, result_urls, status)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                   ON CONFLICT(economic_id) DO UPDATE SET
                     result_urls=COALESCE(excluded.result_urls, result_urls),
            ...
            if result_urls:
                await conn.execute(
                    ...
                    (
                        user_id,
                        json.dumps({"model_key": model_key, "kind": kind}, ensure_ascii=False),
                    ),
                )
            await conn.commit()
            return int(cursor.lastrowid)
    ...
    async def mark_generation_job_ready(
        ...
        *,
        ...
        async with self.connection() as conn:
            ...
            try:
                row = await (await conn.execute(
                    "SELECT economic_id, user_id, model_key, kind FROM generation_jobs WHERE id=?",
                    (job_id,),
                )).fetchone()
                if not row:
                    await conn.rollback()
                ...
                if updated.rowcount == 1:
                    await conn.execute(
                        """UPDATE generation_history
                           SET status='completed', result_urls=?, updated_at=CURRENT_TIMESTAMP
                           WHERE economic_id=?""",
                        (json.dumps(result_urls, ensure_ascii=False), int(row["economic_id"])),
                    )
                    await conn.execute(
                        """INSERT INTO funnel_events (user_id, event, context_json)
                           VALUES (?, 'generation_completed', ?)""",
        ...
        *,
        ...
        async with self.connection() as conn:
            cursor = await conn.execute(
                """INSERT INTO generation_history
                   (user_id, economic_id, model_key, kind, prompt,
                    settings_json, response_preview, result_urls, status)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                   ON CONFLICT(economic_id) DO UPDATE SET
                     response_preview=COALESCE(excluded.response_preview, response_preview),
                     result_urls=COALESCE(excluded.result_urls, result_urls),
                ...
                (
                    ...
                    (prompt or "")[:6000],
                    json.dumps(generation_settings or {}, ensure_ascii=False),
                    response_preview[:3000] if response_preview else None,
                    json.dumps(result_urls, ensure_ascii=False) if result_urls else None,
                    status,
                ),
            )
            ...
            if cursor.lastrowid:
                return int(cursor.lastrowid)
            row = await (await conn.execute(
                "SELECT id FROM generation_history WHERE economic_id=?",
                (economic_id,),
            )).fetchone()
            return int(row["id"])
    ...
    async def get_generation_history(
        ...
        *,
        ...
        query = "SELECT * FROM generation_history WHERE id=?"
        params: tuple[Any, ...] = (history_id,)
    ...
    async def list_generation_history(
        ...
        async with self.connection() as conn:
            rows = await (await conn.execute(
                ...
                (user_id, max(1, min(30, int(limit)))),
            )).fetchall()
        return [dict(row) for row in rows]
    ...
    async def record_model_outcome(
        self,
        model_key: str,
        *,
        success: bool,
        ...
        """Записывает исход и при массовых ошибках временно выключает модель."""
        async with self.connection() as conn:
            await conn.execute(
                """INSERT INTO model_health_events
                   (model_key, success, error_message) VALUES (?, ?, ?)""",
                (model_key, int(success), (error_message or "")[:2000] or None),
            )
            await conn.commit()
        if success or not settings.MODEL_CIRCUIT_BREAKER_ENABLED:
            return False

        window = max(1, settings.MODEL_CIRCUIT_WINDOW_MINUTES)
        async with self.connection() as conn:
            stats = await (await conn.execute(
                """SELECT COUNT(*) AS total,
                          SUM(CASE WHEN success=0 THEN 1 ELSE 0 END) AS failed
                   FROM model_health_events
                   WHERE model_key=? AND created_at >= datetime('now', ?)""",
                (model_key, f"-{window} minutes"),
            )).fetchone()
            total = int(stats["total"] or 0)
            failed = int(stats["failed"] or 0)
            rate = failed * 100.0 / total if total else 0.0
            if (
                total < max(1, settings.MODEL_CIRCUIT_MIN_ATTEMPTS)
                or rate < settings.MODEL_CIRCUIT_FAILURE_PERCENT
            ):
                return False
            reason = (
                f"Автоотключение: {failed}/{total} ошибок "
                f"за {window} мин ({rate:.1f}%)"
            )
            cooldown = max(1, settings.MODEL_CIRCUIT_COOLDOWN_MINUTES)
            cursor = await conn.execute(
                """UPDATE model_prices
                   SET enabled=FALSE, disabled_reason=?,
                       auto_disabled_at=CURRENT_TIMESTAMP,
                       auto_disabled_until=datetime('now', ?),
                       updated_at=CURRENT_TIMESTAMP
                   WHERE model_key=? AND enabled=TRUE""",
                (reason, f"+{cooldown} minutes", model_key),
            )
            await conn.commit()
        if cursor.rowcount == 1:
            self._model_enabled_cache[model_key] = False
            logger.warning("Модель %s автоматически отключена: %s", model_key, reason)
            return True
        return False

    async def reenable_due_models(self) -> List[str]:
        async with self.connection() as conn:
            rows = await (await conn.execute(
                """SELECT model_key FROM model_prices
                   WHERE enabled=FALSE AND auto_disabled_until IS NOT NULL
                     AND auto_disabled_until <= CURRENT_TIMESTAMP"""
            )).fetchall()
            keys = [str(row["model_key"]) for row in rows]
            for key in keys:
                await conn.execute(
                    """UPDATE model_prices
                       SET enabled=TRUE, disabled_reason=NULL,
                           auto_disabled_at=NULL, auto_disabled_until=NULL,
                           updated_at=CURRENT_TIMESTAMP WHERE model_key=?""",
                    (key,),
                )
            await conn.commit()
        for key in keys:
            self._model_enabled_cache[key] = True
        return keys
    ...
    async def create_support_ticket(
        ...
        *,
        ...
        async with self.connection() as conn:
            last_job = await (await conn.execute(
                """SELECT id, model_key, status, provider_request_id, created_at
                   FROM generation_jobs WHERE user_id=? ORDER BY id DESC LIMIT 1""",
                (user_id,),
            )).fetchone()
            last_payment = await (await conn.execute(
            ...
            user = await (await conn.execute(
                "SELECT username, first_name, tokens FROM users WHERE telegram_id=?",
                (user_id,),
            )).fetchone()
            context = {
                "user": dict(user) if user else {},
                "last_generation": dict(last_job) if last_job else {},
                "last_payment": dict(last_payment) if last_payment else {},
            }
            cursor = await conn.execute(
                """INSERT INTO support_tickets
                   (user_id, category, description, attachment_file_id,
                    attachment_type, context_json)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (
                    user_id,
                    category[:80],
            ...
            public_id = f"SUP-{ticket_id:06d}"
            await conn.execute(
                "UPDATE support_tickets SET public_id=? WHERE id=?",
                (public_id, ticket_id),
            )
            await conn.execute(
                """INSERT INTO support_messages
                   ...
                   VALUES (?, 'user', ?, ?)""",
                (ticket_id, user_id, description[:4000]),
            )
            await conn.execute(
                """INSERT INTO funnel_events (user_id, event, context_json)
                   VALUES (?, 'support_created', ?)""",
                (user_id, json.dumps({"ticket": public_id}, ensure_ascii=False)),
            )
            await conn.commit()
            row = await (await conn.execute(
    ...
    async def list_support_tickets(
        ...
        *,
        ...
        query += " ORDER BY CASE status WHEN 'new' THEN 0 WHEN 'in_progress' THEN 1 ELSE 2 END, updated_at DESC LIMIT ?"
        ...
        async with self.connection() as conn:
            cursor = await conn.execute(
                """UPDATE support_tickets
                   ...
                   WHERE id=?""",
                (status, admin_id, status, ticket_id),
            )
            await conn.commit()
            return cursor.rowcount == 1
        ...
        async with self.connection() as conn:
            await conn.execute(
                """INSERT INTO support_messages
                   (ticket_id, sender_type, sender_id, message)
                   VALUES (?, ?, ?, ?)""",
                (ticket_id, sender_type, sender_id, message[:4000]),
            )
            await conn.execute(
    ...
    async def list_support_messages(
        ...
        async with self.connection() as conn:
            rows = await (await conn.execute(
                ...
                (ticket_id, max(1, min(50, int(limit)))),
            )).fetchall()
    ...
    async def set_model_enabled(self, model_key: str, enabled: bool) -> None:
        ...
        async with self.connection() as conn:
            cursor = await conn.execute(
                """UPDATE model_prices
                   SET enabled=?, disabled_reason=NULL, auto_disabled_at=NULL,
                       auto_disabled_until=NULL, updated_at=CURRENT_TIMESTAMP
                   WHERE model_key=?""",
    ...
    async def get_model_settings(self, model_key: str) -> Optional[Dict[str, Any]]:
        async with self.connection() as conn:
            row = await (await conn.execute(
                """SELECT model_key, token_cost, enabled, provider_cost_rub,
                          min_margin_percent, cost_source, disabled_reason,
                          auto_disabled_at, auto_disabled_until, updated_at
                   FROM model_prices WHERE model_key=?""",
                (model_key,),
            )).fetchone()
        return dict(row) if row else None
