...
class DatabaseConnectionManager:
    ...
    async def refresh_runtime_caches(self) -> None:
        model_rows = await (await self._pool.execute(
            "SELECT model_key, enabled FROM model_prices"
        )).fetchall()
    ...
    async def reserve_generation(
        ...
        async with self.connection() as conn:
            ...
            try:
                ...
                # Для конфигурационных цен (видео: качество/разрешение/звук/длительность)
                # сумма уже рассчитана и подтверждена пользователем. В этом случае
                # статическая цена из model_prices не должна её подменять.
                actual_cost = (
                    int(token_cost)
    ...
    async def get_model_settings(self, model_key: str) -> Optional[Dict[str, Any]]:
        async with self.connection() as conn:
            row = await (await conn.execute(
                "SELECT model_key, token_cost, enabled, updated_at FROM model_prices WHERE model_key=?",
                (model_key,),
            )).fetchone()
    ...
    async def get_all_model_settings(self) -> List[Dict[str, Any]]:
        async with self.connection() as conn:
            rows = await (await conn.execute(
                "SELECT model_key, token_cost, enabled, updated_at FROM model_prices ORDER BY model_key"
            )).fetchall()
        return [dict(row) for row in rows]
