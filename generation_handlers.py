...
async def _send_long_text(
    ...
    for start in range(0, len(text), 3900):
        await message.answer(
            html.escape(text[start:start + 3900])
        )
...
async def select_image_model(
    ...
    await state.set_state(
        ImageGenerationStates.choosing_input
    )
    await state.update_data(
        model_key=model_key,
        input_image=None,
...
async def receive_input_image(
    ...
    await state.set_state(next_state)
    await message.answer(
        "Фото принято. Теперь введите промпт."
    )
...
async def generate_media(
    ...
    await message.answer(
        ...
        f"Режим: "
        f"{'🖼 фото → изображение' if data.get('input_image_path') else '📝 текст → изображение'}"
        "\n\nТокены будут списаны только "
        "после подтверждения.",
