...
async def select_image_mode(
    ...
    title = (
        "📝 <b>Изображение по описанию</b>"
        if mode == "text"
        else "📸 <b>Изображение по фотографии</b>"
    )
    hint = (
        "Выберите модель. После этого бот попросит текстовое описание."
        if mode == "text"
        else "Выберите модель. После этого бот попросит загрузить фотографию."
    )
