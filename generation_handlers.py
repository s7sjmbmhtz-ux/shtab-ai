...
def _telegram_file_url(bot: Bot, file_path: str) -> str:
    return f"https://api.telegram.org/file/bot{settings.BOT_TOKEN}/{file_path}"


async def _photo_url(message: Message, bot: Bot) -> str:
    ...
    return _telegram_file_url(bot, telegram_file.file_path)
...
async def open_video_models(message: Message, state: FSMContext) -> None:
    await _ensure_user(message)
    await state.clear()
    await message.answer("Выберите видеомодель:", reply_markup=get_video_models_keyboard())
...
async def package_selected(callback: CallbackQuery) -> None:
    ...
    await callback.message.answer(
        f"Вы выбрали <b>{package['title']}</b>: {package['tokens']} 💎 за {package['price_rub']} ₽.\n\n"
        "Подключение платёжной формы будет добавлено отдельным пакетом."
    )
...
async def select_video_model(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if model.supports_input_image:
        ...
        await callback.message.answer(f"{model_caption(model_key)}\n\nВведите промпт для видео.")
...
async def receive_input_image(message: Message, state: FSMContext, bot: Bot) -> None:
    url = await _photo_url(message, bot)
    data = await state.get_data()
    model = get_model(data["model_key"])
    next_state = ImageGenerationStates.waiting_prompt if model.kind == GenerationKind.IMAGE else VideoGenerationStates.waiting_prompt
    await state.update_data(input_image=url)
    await state.set_state(next_state)
...
async def generate_media(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        if model.kind == GenerationKind.IMAGE:
            ...
            await message.answer_video(urls[0], caption=f"✅ Готово · {model.title}", supports_streaming=True)
        await state.clear()
    except InsufficientBalanceError as exc:
        await state.clear()
        await status.edit_text(
        ...
        if charge is not None:
            # create_media_task возвращает charge только после успешного запуска.
            # Ошибка ожидания/выдачи результата тоже должна вернуть оплату.
