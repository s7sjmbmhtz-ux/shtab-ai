...
RESULT_WRAPPER_KEYS = ("data", "request")


def _collect_urls(value: Any) -> list[str]:
    """Возвращает только ссылки из фактического результата GenAPI.

    Поля ``parameters`` и ``full_response`` намеренно не обходятся: в них
    провайдер часто возвращает URL исходной фотографии. Берём первый
    непустой блок результата, поэтому загруженное фото не отправляется
    пользователю повторно вместе с генерацией.
    """
    ...
    for key in RESULT_URL_KEYS:
        if key not in value:
            continue
        urls = _collect_urls(value[key])
        if urls:
            return list(dict.fromkeys(urls))

    for key in RESULT_WRAPPER_KEYS:
        nested = value.get(key)
        if nested is None:
            continue
        urls = _collect_urls(nested)
        if urls:
            return list(dict.fromkeys(urls))
...
async def select_image_mode(
    ...
    await callback.message.answer(
        f"{title}\n\nВыберите модель — цены идут по возрастанию:",
...
async def select_image_model(
    ...
    await callback.answer()
    await callback.message.edit_text(
        model_caption(model_key, image_mode=mode),
...
async def request_input_image(
    ...
    if (
        generation_service.requires_public_url(model_key)
        and not public_media_service.is_configured
    ):
        await callback.answer(
            "Для работы по фото не настроен публичный домен BotHost.",
            show_alert=True,
        )
        return
...
async def choose_cartoonify_strength(
    ...
    # GenAPI требует scale >= 1. Значения ниже 1 дают HTTP 422.
    options = {
        "light": (1.0, "🙂 Лёгкий"),
        "medium": (1.25, "🎨 Средний"),
        "strong": (1.5, "🧸 Сильный"),
