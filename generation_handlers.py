...
async def open_text_models(
    ...
    await funnel_service.track(message.from_user.id, "catalog_open", {"kind": "text"})
    await state.clear()
    await message.answer(
...
async def open_image_models(
    ...
    await _ensure_user(message)
    await funnel_service.track(message.from_user.id, "catalog_open", {"kind": "image"})
    await state.clear()
...
async def open_video_models(
    ...
    await _ensure_user(message)
    await funnel_service.track(message.from_user.id, "catalog_open", {"kind": "video"})
    await state.clear()
...
async def show_packages(message: Message) -> None:
    await _ensure_user(message)
    await funnel_service.track(message.from_user.id, "packages_open")
    await message.answer(
...
async def select_text_model(
    ...
    await funnel_service.track(
        ...
        "model_selected",
        {"model_key": model_key, "kind": model.kind.value},
    )
...
async def confirm_generation(
    ...
    if await db_manager.has_active_generation_job(user_id):
        await callback.answer(
            "Предыдущая генерация сохранена и ещё выполняется. "
...
async def _execute_media(
    ...
    *,
    ...
    try:
        ...
        provider_cost_rub = None
        if model.kind == GenerationKind.VIDEO:
            selection = data.get("video_selection")
            if selection:
                provider_cost_rub = effective_video_cost_rubles(
                    model_key,
                    selection,
                )
        ...
        job_id = await generation_job_service.register(
            user_id=user_id,
            ...
            prompt=prompt,
            generation_settings={
                "video_selection": data.get("video_selection"),
                "media_overrides": overrides,
                "duration": duration,
                "token_cost": data.get("token_cost"),
                "has_input_image": bool(data.get("input_image_path")),
                "image_mode": data.get("image_mode"),
            },
        )
        urls = await generation_job_service.wait_for_result(job_id, task)
        ...
        await generation_job_service.mark_delivered(job_id)
        ...
        if job_id is not None:
            await generation_job_service.defer(
                job_id,
                RuntimeError("Процесс остановлен; задача будет восстановлена"),
            )
        ...
        pending_saved = False
        if job_id is not None:
            if isinstance(exc, (GenAPITaskFailedError, EmptyGenerationResultError)):
                await generation_job_service.fail_and_refund(
                    job_id,
                    "Подтверждённая ошибка генерации",
                )
            else:
                await generation_job_service.defer(job_id, exc)
                pending_saved = True
        elif charge is not None:
        ...
        if model.kind == GenerationKind.IMAGE and not pending_saved:
            await state.set_state(ImageGenerationStates.choosing_mode)
            reply_markup = get_image_mode_keyboard()
        else:
            await state.clear()
            reply_markup = None if pending_saved else get_video_models_keyboard()

        await status.edit_text(
            (
                "⌛ <b>Генерация сохранена</b>\n\n"
                "Провайдер ещё обрабатывает запрос или временно не отвечает. "
                "Бот продолжит проверку автоматически и пришлёт результат. "
                "Повторно запускать запрос не нужно."
                if pending_saved
                else _friendly_media_error(exc)
            ),
        ...
        await _stop_progress(progress_task)
        generation_job_service.release_local(job_id)
