...
def _video_aspect_overrides(
    ...
    if model_key == "runway-gen4":
        return {"ratio": "720:1280" if vertical else "1280:720"}
...
def _video_settings_keyboard(model_key: str, selection: dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    def add_choice_row(field: str, choices: tuple[Any, ...]) -> None:
        ...
        rows.append([
            InlineKeyboardButton(
                ...
                callback_data=f"video_option:{field}:{choice.value}",
            )
    ...
    if len(options.durations) > 1:
        rows.append([
            InlineKeyboardButton(
                ...
                callback_data=f"video_option:duration:{seconds}",
            )
    ...
    rows.append([InlineKeyboardButton(text="➡️ Продолжить", callback_data="video_options_continue")])
...
async def select_image_model(
    ...
    await callback.message.edit_text(
        model_caption(model_key, image_mode=mode) + free_notice,
