...
async def select_image_model(
    ...
    if (
        ...
        await callback.answer(
            "Для этой модели включите «Использовать домен» в BotHost, "
            "дождитесь статуса Online и повторите выбор.",
            show_alert=True,
        )
        return
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            if (
                model_key == "seededit"
                and "Произошла ошибка" in str(exc)
            ):
                await status.edit_text(
                    "⏳ SeedEdit временно не ответил. Повторяю один раз…"
                )
                retry_task = await generation_service.retry_media_task(
                    model_key,
                    prompt,
                    input_image=input_image,
                    overrides=overrides,
                )
                result = await _complete_media_task(retry_task)
            else:
                raise
