    ...
    *,
    ...
    for model in list_models(kind):
        if kind == GenerationKind.IMAGE:
            # Служебные модели маркетплейсов не показываем
            # в обычном разделе создания изображений.
            if model.key.startswith("marketplace-"):
                continue
            if image_mode == "photo" and not model.supports_input_image:
                continue
            if image_mode == "text" and model.requires_input_image:
                continue
