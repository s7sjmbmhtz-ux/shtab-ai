...
def _model_price(model_key: str) -> int:
    model = get_model(model_key)
    return db_manager.get_model_price_cached(model.key, model.token_cost)


def _video_sort_price(model_key: str) -> int:
    try:
        return video_min_cost_tokens(model_key)
    except (KeyError, ValueError):
        return _model_price(model_key)
...
def _models_keyboard(
    ...
    *,
    ...
    models = [model for model in models if db_manager.is_model_enabled_cached(model.key)]
    if kind == GenerationKind.VIDEO:
        models = sorted(
            models,
            key=lambda model: (
                _video_sort_price(model.key),
                model.title.lower(),
            ),
        )
    else:
        models = sorted(
            models,
            key=lambda model: (_model_price(model.key), model.title.lower()),
        )
    ...
    for model in models:
        ...
        if kind == GenerationKind.VIDEO:
            ...
            try:
                ...
                price = f"{_model_price(model.key)} 💎"
            text = f"{icon} {model.title} · {modes} · {price}"
        elif kind == GenerationKind.IMAGE:
            text = f"{model.title} · {_model_price(model.key)} 💎"
        else:
            text = f"{icon} {model.title} · {_model_price(model.key)} 💎"
...
def get_cartoonify_strength_keyboard() -> InlineKeyboardMarkup:
    price = _model_price("cartoonify")
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text=f"🙂 Лёгкий эффект · {price} 💎",
                    callback_data="cartoonify_strength:light",
                )
            ],
            [
                InlineKeyboardButton(
                    text=f"🎨 Средний эффект · {price} 💎",
                    callback_data="cartoonify_strength:medium",
                )
            ],
            [
                InlineKeyboardButton(
                    text=f"🧸 Сильный эффект · {price} 💎",
...
def get_token_packages_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    packages = db_manager.get_token_packages_cached(enabled_only=True)
    if not packages and not db_manager.get_token_packages_cached(enabled_only=False):
        packages = [
    ...
    for package in packages:
        builder.button(
            ...
            callback_data=f"buytokens:{package['package_key']}",
        )
    builder.adjust(1)
    return builder.as_markup()
