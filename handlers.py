...
WELCOME = (
    ...
    "Здесь собраны инструменты искусственного интеллекта для работы, "
    "бизнеса и творчества — без отдельных подписок на разные сервисы.\n\n"
    "<b>Что можно сделать:</b>\n"
    "💬 подготовить текст, идею, анализ или деловой документ;\n"
    "🖼 создать изображение или обработать фотографию;\n"
    "🎬 сгенерировать видео по описанию или оживить фото;\n"
    "🛒 оформить карточку товара для маркетплейса;\n"
    "📈 получить материалы для маркетинга и продаж.\n\n"
...
async def _main_menu_text(
    ...
    *,
    ...
    """Формирует полезное приветствие вместо технического заголовка меню."""
    name = html.escape((first_name or "").strip())
    if first_visit:
        title = f"🚀 <b>Добро пожаловать{name and ', ' + name}!</b>"
    else:
        title = f"👋 <b>С возвращением{name and ', ' + name}!</b>"

    free = await db_manager.get_free_credits(user_id)
    if _is_admin(user_id):
        balance_line = "👑 Режим администратора: <b>без списания токенов</b>"
    else:
        balance = await token_repository.get_user_tokens(user_id)
        balance_line = f"💎 Баланс: <b>{balance} токенов</b>"

    return (
        f"{title}\n\n"
        "<b>ШТАБ AI</b> — единое пространство для текста, изображений, "
        "видео и задач бизнеса. Вам не нужно разбираться в десятках сервисов: "
        "выберите задачу, а бот предложит подходящие модели и покажет цену до запуска.\n\n"
        "<b>Возможности:</b>\n"
        "💬 AI-ассистент для текстов, идей и анализа\n"
        "🖼 Создание и обработка изображений\n"
        "🎬 Видео по описанию или фотографии\n"
        "🛒 Карточки товаров для маркетплейсов\n"
        "🏢 Инструменты для продаж и маркетинга\n"
        "🕘 История и повтор готовых генераций\n\n"
        f"{balance_line}\n"
        "🎁 Бесплатно осталось: "
...
async def start_command(
    ...
    await funnel_service.track(message.from_user.id, "bot_started")
    accepted = await db_manager.is_onboarding_accepted(message.from_user.id)
    if accepted:
        await message.answer(
            await _main_menu_text(
                message.from_user.id,
                message.from_user.first_name,
                first_visit=False,
            ),
...
async def welcome_start(
    ...
    await funnel_service.track(callback.from_user.id, "onboarding_completed")
