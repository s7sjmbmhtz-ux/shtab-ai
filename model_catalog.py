...
MODELS: dict[str, ModelSpec] = {
    ...
    "flux-schnell": ModelSpec(
        ...
        description="Быстрая генерация идей, эскизов и простых изображений.",
        tier="economy", speed="очень высокая", quality="базовое",
        use_cases=("идеи", "эскизы", "быстрые варианты"),
    ),
    "flux-dev": ModelSpec(
        ...
        description="Оптимальный вариант для рекламы, соцсетей и карточек товара.",
        tier="standard", speed="средняя", quality="высокое",
        use_cases=("реклама", "соцсети", "карточки товара"),
    ),
    "flux-pro": ModelSpec(
        ...
        {"translate_input": True, "width": 1024, "height": 1024,
         ...
         "enable_safety_checker": True},
        supports_input_image=True,
        description="Премиальная генерация коммерческих и художественных изображений.",
        tier="premium", speed="средняя", quality="максимальное",
        use_cases=("реклама", "баннеры", "коммерческие изображения"),
    ),

    "nano-banana-lite": ModelSpec(
        "nano-banana-lite", "Nano Banana Lite", GenerationKind.IMAGE,
        "/api/v1/networks/nano-banana-lite", "v1", 40,
        {
            "is_sync": False,
            "aspect_ratio": "1:1",
            "model": "v1",
        ...
        supports_input_image=True,
        description="Быстрая универсальная модель для создания изображений и работы по фото.",
        tier="economy", speed="высокая", quality="хорошее",
    ),
    "nano-banana-2": ModelSpec(
        "nano-banana-2", "Nano Banana 2", GenerationKind.IMAGE,
        "/api/v1/networks/nano-banana-2", None, 140,
        {
            "is_sync": False,
            "aspect_ratio": "1:1",
            "resolution": "1K",
        ...
        supports_input_image=True,
        description="Улучшенная модель для более точных и качественных изображений.",
        tier="premium", speed="средняя", quality="максимальное",
    ),

    # Проверенные модели редактирования по фотографии
    "seededit": ModelSpec(
        "seededit", "🎨 SeedEdit", GenerationKind.IMAGE,
        "/api/v1/networks/seededit", "seededit", 45,
        {"translate_input": True},
        supports_input_image=True, requires_input_image=True,
        description="Редактирование фотографии по текстовому запросу.",
        tier="standard",
    ),
    "flux-kontext": ModelSpec(
        "flux-kontext", "⭐ Flux Kontext", GenerationKind.IMAGE,
        "/api/v1/networks/flux-kontext", "max", 70,
        {"is_sync": False, "translate_input": True, "guidance_scale": 3.5,
         "num_images": 1, "output_format": "jpeg", "safety_tolerance": 6},
        supports_input_image=True, requires_input_image=True,
        description="Локальное редактирование фотографии по текстовому запросу.",
        tier="premium",
    ),
    "qwen-image-edit-plus": ModelSpec(
        "qwen-image-edit-plus", "🖌 Qwen Image Edit Plus", GenerationKind.IMAGE,
        "/api/v1/networks/qwen-image-edit-plus", None, 55,
        {"translate_input": True, "negative_prompt": "disfigured, ugly, deformed",
         "width": 992, "height": 992, "num_images": 1, "output_format": "png",
         "guidance_scale": 4, "num_inference_steps": 50,
         "enable_safety_checker": False, "acceleration": "none"},
        supports_input_image=True, requires_input_image=True,
        description="Редактирование изображения по текстовой инструкции.",
        tier="premium",
    ),
