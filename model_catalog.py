...
MODELS: dict[str, ModelSpec] = {
    ...
    "gpt-image-2": ModelSpec(
        ...
        "/api/v1/networks/gpt-image-2", None, 150,
        {
            "quality": "high",
            "image_size": "1024x768",
            "num_images": 1,
        ...
        description="Лучший универсальный вариант.",
        tier="premium", speed="средняя", quality="максимальное",
    ),
    "nano-banana-2": ModelSpec(
        "nano-banana-2", "🎨 Nano Banana 2", GenerationKind.IMAGE,
        "/api/v1/networks/nano-banana-2", None, 250,
        {
            "is_sync": False,
            "aspect_ratio": "1:1",
            "resolution": "1K",
        ...
        # Формат загрузки фото в документации GenAPI не раскрыт.
        # До подтверждения API используем модель только по описанию.
        supports_input_image=False,
        description="Стилизация и креатив.",
        tier="premium", speed="средняя", quality="максимальное",
    ),
    "flux-2-pro": ModelSpec(
        "flux-2-pro", "⭐ Flux 2 Pro", GenerationKind.IMAGE,
        "/api/v1/networks/flux-2-pro", None, 150,
        {
    ...
    "nano-banana-lite": ModelSpec(
        ...
        # Входное фото временно отключено: GenAPI отклоняет multipart-массив.
        supports_input_image=False,
