...
class GenAPITaskFailedError(GenAPIError):
    """Провайдер подтвердил окончательную ошибку задачи."""


class GenAPITaskTimeoutError(GenAPIError):
    """Ожидание истекло, но задача может продолжать выполняться."""
        ...
        *,
        ...
        while asyncio.get_running_loop().time() < deadline:
            ...
            if status in failed_statuses:
                ...
                raise GenAPITaskFailedError(str(error))

            await asyncio.sleep(interval)

        raise GenAPITaskTimeoutError(
