...
class GenAPIClient:
    ...
    def _parse_response(
        ...
        if response.is_error:
            if response.status_code == 401:
                raise GenAPIError(
                    "GenAPI отклонил API-ключ. "
                    "Проверьте GENAPI_API_KEY."
                )
    ...
    async def wait_for_result(
        ...
        *,
        ...
        """
        Получение результата асинхронной задачи GenAPI.

        Раньше здесь использовался POST, из-за чего GenAPI
        возвращал HTTP 405 Method Not Allowed.
        """
        ...
        endpoint = (
            f"/api/v1/request/get/{request_id}"
        )
