...
class GenerationJobService:
        ...
        *,
        ...
        if urls:
            with contextlib.suppress(Exception):
                await db_manager.record_model_outcome(model_key, success=True)
    ...
    async def wait_for_result(
        ...
        transitioned = await db_manager.mark_generation_job_ready(job_id, urls)
        if transitioned:
            job = await db_manager.get_generation_job(job_id)
            if job:
                with contextlib.suppress(Exception):
    ...
    async def fail_and_refund(
        ...
        *,
        ...
        job = await db_manager.get_generation_job(job_id)
        refunded = await db_manager.refund_generation_job(job_id, reason)
        if refunded and count_failure and job:
            with contextlib.suppress(Exception):
                await db_manager.record_model_outcome(
                    str(job["model_key"]),
                    success=False,
    ...
    async def schedule_recovery(self, job_id: int) -> bool:
        """Немедленно ставит сохранённую задачу на повторную проверку."""
        row = await db_manager.get_generation_job(job_id)
        if not row or row.get("status") not in {"processing", "ready"}:
            return False
        ...
        task = asyncio.create_task(
            ...
            name=f"generation-manual-recovery-{job_id}",
        )
