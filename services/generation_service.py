...
# Для video-endpoint используем JSON и временную публичную ссылку.
# Это важно: multipart превращает bool-параметры (например generate_audio)
# в строки и некоторые endpoint отклоняют такой запрос. Кроме того,
# image_urls должен быть настоящим JSON-массивом, а не одним file-part.
...
class GenerationService:
    ...
    async def _media_payload(
        ...
        # Veo 3.1 требует явный режим: img2video только при наличии
        # ровно одного изображения, иначе используется txt2video.
        if model_key == "veo-3-1":
            payload["mode"] = "img2video" if image else "txt2video"
        ...
        if image:
            if model.kind == GenerationKind.IMAGE:
                ...
                public_url = await public_media_service.register(image)
                if transport == "url":
                    payload[field] = public_url
                elif transport == "url_array":
                    # Для img2video GenAPI требует ровно один элемент.
                    payload[field] = [public_url]
                else:
                    raise ValueError(
        ...
        if end_image:
            ...
            public_url = await public_media_service.register(end_image)
            if transport == "url":
                payload[field] = public_url
            elif transport == "url_array":
                payload[field] = [public_url]
