...
class OperationsService:
    ...
    async def _run_checks(self) -> None:
        reenabled = await db_manager.reenable_due_models()
        for model_key in reenabled:
            await self._alert(
                f"model_reenabled:{model_key}",
                "✅ <b>Модель автоматически включена после паузы</b>\n"
                f"<code>{html.escape(model_key)}</code>",
            )
        async with db_manager.connection() as conn:
            ...
            disabled_models = await (await conn.execute(
                """SELECT model_key, disabled_reason, auto_disabled_until
                   FROM model_prices
                   WHERE enabled=FALSE AND auto_disabled_at IS NOT NULL
                     AND auto_disabled_at >= datetime('now', '-15 minutes')"""
            )).fetchall()
        ...
        for row in disabled_models:
            key = str(row["model_key"])
            await self._alert(
                f"model_disabled:{key}",
                "⛔ <b>Модель автоматически отключена</b>\n"
                f"<code>{html.escape(key)}</code>\n"
                f"{html.escape(str(row['disabled_reason'] or 'Рост ошибок'))}\n"
                f"Повторное включение: <code>{html.escape(str(row['auto_disabled_until']))}</code>",
            )
