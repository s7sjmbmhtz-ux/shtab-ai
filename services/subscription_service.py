...
async def get_user_tariff(user_id: int) -> Tariff:
    ...
    tariff_str = await db_manager.get_user_tariff(user_id)
    return Tariff(tariff_str)


async def set_user_tariff(user_id: int, tariff: Tariff) -> bool:
    """Устанавливает тариф пользователю."""
    return await db_manager.set_user_tariff(user_id, tariff.value)
...
async def get_subscription_end_date(user_id: int) -> Optional[datetime]:
    ...
    end_date_str = await db_manager.get_subscription_end_date(user_id)
    if end_date_str:
