...
from model_catalog import MODELS, GenerationKind, get_model
...
async def set_model_price(message: Message, command: CommandObject) -> None:
    ...
    if model.kind == GenerationKind.VIDEO:
        await message.answer(
            "Цена видео рассчитывается автоматически по качеству, разрешению, "
            "длительности и звуку. Команда /price для видеомоделей не применяется."
        )
        return
    if token_cost <= 0:
        await message.answer("Цена модели должна быть больше нуля.")
        return
...
async def admin_models(message: Message) -> None:
    ...
    for spec in MODELS.values():
        price = db_manager.get_model_price_cached(spec.key, spec.token_cost)
        lines.append(f"<code>{spec.key}</code> — {spec.title}: {price} 💎")
    await message.answer("\n".join(lines))
