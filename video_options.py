...
LANDSCAPE_PORTRAIT_ASPECTS = (
    Choice("horizontal", "Горизонтальный 16:9"),
    Choice("vertical", "Вертикальный 9:16"),
)
...
VIDEO_OPTIONS: Mapping[str, VideoOptions] = {
    ...
    "ltx-2-3": VideoOptions(
        # В текущей схеме подтверждён режим Pro. Fast включается только
        # после отдельного успешного приёмочного запроса к GenAPI.
        qualities=(Choice("pro", "Pro"),),
        resolutions=(
            Choice("1080p", "1080p"),
            Choice("1440p", "1440p"),
            Choice("2160p", "2160p"),
        ...
        aspects=LANDSCAPE_PORTRAIT_ASPECTS,
    ),
    ...
    "kling-v3": VideoOptions(
        # Точное API-значение Standard не подтверждено; оставлен Pro,
        # который указан в рабочей схеме endpoint.
        qualities=(Choice("pro", "Pro"),),
        durations=(5,),
        audio_choices=(Choice("off", "Без звука"), Choice("on", "Со звуком")),
        default_quality="pro",
...
def video_cost_tokens(model_key: str, selection: Mapping[str, Any]) -> int:
    return _tokens_from_rubles(effective_video_cost_rubles(model_key, selection))


def effective_video_cost_rubles(
    model_key: str,
    selection: Mapping[str, Any],
) -> float:
    """Учитывает ручное обновление себестоимости без правки таблиц кода.

    В админке задаётся стоимость конфигурации по умолчанию. Остальные варианты
    масштабируются тем же коэффициентом, после чего цена в 💎 пересчитывается.
    """
    known_cost = video_cost_rubles(model_key, selection)
    try:
        from database import db_manager

        if db_manager.get_model_cost_source_cached(model_key) != "manual":
            return known_cost
        configured = db_manager.get_model_provider_cost_cached(model_key)
        base_known = video_cost_rubles(
            model_key,
            default_video_selection(model_key),
        )
        if configured is None or configured <= 0 or base_known <= 0:
            return known_cost
        return known_cost * configured / base_known
